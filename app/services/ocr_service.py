...
def _append_text_candidate(value: Any, output: list[str]) -> None:
    ...
    if isinstance(value, dict):
        for key in ("natural_text", "text", "content"):
            nested = value.get(key)
            if nested is not None:
                _append_text_candidate(nested, output)
        return

    if isinstance(value, list):
        for item in value:
            _append_text_candidate(item, output)
    ...
    if isinstance(value, dict):
        for key in ("natural_text", "text", "content"):
            ...
            if isinstance(candidate, str):
                # Some providers return JSON as a string in "content".
                try:
                    parsed_candidate = json.loads(candidate)
                except json.JSONDecodeError:
                    _append_text_candidate(candidate, output)
                else:
                    _append_text_candidate(parsed_candidate, output)
            elif candidate is not None:
                _append_text_candidate(candidate, output)

        for nested in value.values():
            if isinstance(nested, (dict, list)):
                _collect_text_candidates(nested, output)
        return
...
def extract_text_from_result(payload: dict[str, Any]) -> str:
    ...
    _collect_text_candidates(payload, fallback_texts)
